const URL =
  'https://randomuser.me/api/?results=30&inc=name,dob,gender,location,phone,picture';
...
const sortAge = users => {
  const { value: valueAge } = document.getElementById('sort-age');

  if (valueAge === 'increase') {
    createCards(users.sort((a, b) => a.dob.age - b.dob.age));
  } else if (valueAge === 'descrease') {
    createCards(users.sort((a, b) => b.dob.age - a.dob.age));
  }
};

const sortName = users => {
  const { value: valueName } = document.getElementById('sort-name');

  if (valueName === 'increase') {
    createCards(users.sort((a, b) => (a.name.first < b.name.first ? -1 : 1)));
  } else if (valueName === 'descrease') {
    createCards(users.sort((a, b) => (a.name.first > b.name.first ? -1 : 1)));
  }
};
